...
function getOrdersByCustomerId($customer_id) {
   ...
   SELECT orders.number_of_order, orders.date, 
               SUM(order_line.count * order_line.price) AS total_sum
        FROM orders
        JOIN order_line ON orders.id = order_line.id_order
        WHERE orders.id_customers = ?
    ...
    $sql = "
        SELECT 
            ...
            SUM(ol.count * COALESCE((
                ...
                WHERE s.id_product = ol.id_product
                  AND p.date_start <= o.date
                  AND (p.date_end IS NULL OR p.date_end >= o.date)
            ), 0)) AS total_cost
        FROM orders o
        JOIN order_line ol ON o.id = ol.id_order
        WHERE o.id_customers = ?
